...
class TrafficLightBicycle:
    ...
    def __init__(self, red_interval=RED_INTERVAL,rotated=False,initial_state="red"):
        # Load the bicycle traffic light images
        self.red_light = pygame.image.load('images/red_bicycle_traffic_light_2.png')
        self.yellow_light = pygame.image.load('images/yellow_bicycle_traffic_light_2.png')
        self.green_light = pygame.image.load('images/green_bicycle_traffic_light_2.png')

        if rotated: # Check if the rotated flag is True
            self.red_light = pygame.transform.rotate(self.red_light, 0)
            self.yellow_light = pygame.transform.rotate(self.yellow_light, 0)
            self.green_light = pygame.transform.rotate(self.green_light, 0)

        # Initialize the state of the bicycle traffic light
    ...
    def draw(self, screen, position):
        if self.state == "red": # Check if the state of the bicycle light is red
    ...
    #Check how much time has passed since program initialization
    ...
    def switch_light(self,red_interval=RED_INTERVAL, yellow_interval=YELLOW_INTERVAL, green_interval=GREEN_INTERVAL):
        if self.state == "red": # Check if the state of bicycle light is red
...
traffic_light1 = TrafficLightBicycle(initial_state="red", red_interval=4000)
